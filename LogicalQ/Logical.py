...
class LogicalCircuit(QuantumCircuit):
    ...
    def __init__(
        ...
        # @TODO - implement support for mixed-type circuits
        self.logical_qubit_type = "qec" if self.d % 2 == 1 else "qed"
        ...
        self.add_logical_qubits(self.n_logical_qubits, logical_qubit_types=self.logical_qubit_type)
    ...
    def add_logical_qubits(
        ...
        logical_qubit_types: str | Iterable[str] = "qec"
    ):
        ...
        # If a single str is provided, all are of the same type
        if isinstance(logical_qubit_types, str):
            logical_qubit_types = [logical_qubit_types]*logical_qubit_count

        if len(logical_qubit_types) != logical_qubit_count:
            raise ValueError(f"Input for logical_qubit_types has length {len(logical_qubit_types)}; input must either be a single string or an iterable of length matching logical_qubit_count ({logical_qubit_count})")
        ...
        for i in range(current_logical_qubit_count, current_logical_qubit_count + logical_qubit_count):
            ...
            if logical_qubit_types[i] == "qec":
                # QEC cycle indices
                self.qec_cycle_indices_initial[i] = []
                self.qec_cycle_indices_final[i] = []
            elif logical_qubit_types[i] == "qed":
                # QED cycle indices
                self.qed_cycle_indices_initial[i] = []
                self.qed_cycle_indices_final[i] = []
    ...
    def encode(
        ...
        *qubits: Iterable[int],
        ...
        Args:
            qubits: Indices of logical qubits to be encoded.
            max_iterations: Maximum number of encoding attempts.
            initial_states: Initial states in which to encode each qubit.
        """
        ...
        for q, init_state in zip(qubits, initial_states):
            with self.box(label=f"logical.{self.logical_qubit_type}.encode:$\\hat U_{{enc}}$"):
                ...
                if max_iterations > 0:
                    with self.box(label=f"logical.{self.logical_qubit_type}.encoding_verification:$\\hat U_{{enc,verif}}$"):
                        for _ in range(max_iterations):
                            if True:
                                # @TEST - verify that this works
    ...
    def append_qed_cycle(
        ...
        for q in logical_qubit_indices:
            if len(self.data_without_qed) == 0:
                self.data_without_qed = copy.deepcopy(self.data)
            else:
    ...
    def append_qec_cycle(
        ...
        for q in logical_qubit_indices:
            if len(self.data_without_qec) == 0:
                self.data_without_qec = copy.deepcopy(self.data)
            else:
    ...
    def measure(
        ...
        with_error_detection : bool = True,
        ...
        Args:
            ...
            cbit_indices: Classical bits to which to record measurements.
            with_error_detection: Whether to measure syndromes after measurement.
            with_error_correction: Whether to measure and correct syndromes after measurement.
        
        Raises:
            :class:`ValueError`: if `logical_qubit_indices` or `cbit_indices` is not an iterable or if number of qubits and cbits does not match.
        """
        ...
        for q, c in zip(logical_qubit_indices, cbit_indices):
            with self.box(label="logical.qec.measure:$\\hat{M}_\\text{QEC}$"):
                ...
                # logzvec = (logzvec0) % 2 # @TODO - check this generalization attempt
                ...
                if with_error_detection or with_error_correction:
                    # Final syndrome
                    for n in range(self.n_ancilla_qubits):
            ...
            if basis == "physical":
                ...
                if n_logical_qubits and label and stabilizer_tableau:
                    ...
                    for (i, state) in enumerate(states):
                        lqcs[i].encode(range(self.n_logical_qubits), max_iterations=0, initial_states=state)
