...
class LogicalCircuit(QuantumCircuit):
    def __init__(
        ...
        self.logical_qregs = [QuantumRegister(self.n, name=f"ql{i}") for i in range(n_logical_qubits)]
        # Each ancilla quantum register stores the three quantum bits needed for measurements
        self.ancilla_qregs = [QuantumRegister(3, name=f"qa{i}") for i in range(n_logical_qubits)]
        # Each measurement classical register stores the three classical bits needed for measurements
        self.measure_cregs = [ClassicalRegister(3, name=f"cm{i}") for i in range(n_logical_qubits)]
    ...
    def add_logical_qubits(self, qubit_count):
        ...
        for i in range(current_qubit_count, current_qubit_count + qubit_count):
            logical_qreg_i = QuantumRegister(self.n, name=f"ql{i}")
            ancilla_qreg_i = QuantumRegister(3, name=f"qa{i}")
            measure_creg_i = ClassicalRegister(3, name=f"cm{i}")
    ...
    def perform_qec_cycle(self, qubits=[]):
        if len(qubits) == 0:
            qubits = [q for q in range(self.logical_qregs)]
