...
class LogicalCircuit(QuantumCircuit):
    ...
    def group_stabilizers(self):
        ...
        self.flagged_stabilizers_1 = [3, 1, 2]
        self.flagged_stabilizers_2 = [4, 5, 0]
    ...
    def perform_qec_cycle(self, logical_qubit_indices=[]):
        ...
        for q in logical_qubit_indices:
            ...
            self.measure_syndrome_diff([q], self.flagged_stabilizers_1, flagged=True)
            ...
            with self.if_test(expr.equal(self.flagged_syndrome_diff_cregs[q], 0)):
                self.measure_syndrome_diff([q], self.flagged_stabilizers_2, flagged=True)
            ...
            with self.if_test(expr.not_equal(self.flagged_syndrome_diff_cregs[q], 0)):
                self.measure_syndrome_diff([q], self.x_stabilizers, flagged=False)
                self.measure_syndrome_diff([q], self.z_stabilizers, flagged=False)
        
                self.apply_decoding([q], self.x_stabilizers, with_flagged=False)
                self.apply_decoding([q], self.z_stabilizers, with_flagged=False)
                self.apply_decoding([q], self.x_stabilizers, with_flagged=True)
                self.apply_decoding([q], self.z_stabilizers, with_flagged=True)
    ...
    def measure(self, logical_qubit_indices, cbit_indices):
        ...
        for q, c in zip(logical_qubit_indices, cbit_indices):
            ...
            #Final syndrome diff
            for n in range(self.n_ancilla_qubits):
                with super().if_test(self.cbit_xor([self.current_syndrome_cregs[q][n], self.prev_syndrome_cregs[q][self.x_stabilizers[n]]])) as _else:
